...
dateparse = lambda x: dt.datetime.strptime(' '.join(x), '%m/%d/%Y %H:%M')
df = pd.read_csv('D:\data.txt', header = None)
df.index = df[[0,1]].apply(lambda x: dateparse(x), axis = 1)
del df[0]
del df[1]
...
#         print(self.entry, " ", self.curr_price)
...
def run(df,fname):
    ...
    model.compile(tf.keras.optimizers.SGD(lr=.005), "mse")
